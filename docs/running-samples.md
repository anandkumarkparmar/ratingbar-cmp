...
All platform launchers host the same shared `SampleApp()` implementation from `:samples:common`, which depends on the local `:ratingbar-cmp` library module via a direct `project()` dependency.
